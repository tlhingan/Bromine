...
uint16_t ReadPower = 2500;
uint16_t WritePower = 0;
...
void importReaderConfig()
{
   ...
   int j;
   int k;
   printf("%s%c%c", "Content-Type:text/plain;charset=us-ascii", 13, 10);
 ...
 str_modulation, str_delimiter, str_region, str_hopTime, str_HopTable);
      ...
      while (str_OutputList[j] != '\0')
      {
         ...
         arrayOutputList[k] = atoi(str_str_OutpulList);
         k++;
      }
      ...
      while (str_PortSwitchGPOs[j] != '\0')
      {
         ...
         arrayPortSwitchGPOs = atoi(str_str_PortSwitchGPOs);
         k++;
      }
      ...
      while (str_HopTable[j] != '\0')
      {
         ...
         arrayHopTable = atoi(str_str_HopTable);
         k++;
      }
